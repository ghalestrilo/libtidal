...
-- | FIXME: Song Structure
let structureSong st (p:ps) =  (st,(st+(fst p)),snd p) : (structureSong ((fst p)+st) ps )
let structureSong st [] = []
let structureSong _ _ = []
let measures n p ms = (n, p):ms
let loopend n p = measures n p []
let songLoop ms = seqPLoop $ structureSong 0 ms
